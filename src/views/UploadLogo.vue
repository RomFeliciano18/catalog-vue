<script setup>
import { useProcessLogo } from '@/composables/useProcessLogo';
...
const handleFileUpload = async (files) => {
  ...
  if (fileType.startsWith('image/') || fileType === 'image/svg+xml') {
    ...
    try {
      ...
      console.log(processLogo.original);
    }
...
const test = () => {
  console.log(storeLB.lookbookFeatures);
};
</script>
        ...
        <template v-if="processLogo.original && !isProcessing">
          ...
          <div class="flex h-full w-full items-center justify-evenly gap-2">
            <div class="flex flex-col gap-1">
              <p class="mb-2 font-bold">Original:</p>
...
.preview-image-background {
  background-image: url(/src/assets/images/bg-transparent.jpg);
}
.loader {
  width: 64px;
  height: 64px;
